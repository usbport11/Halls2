...
std::vector<MEdge> MDelaunay::CreateMSTEdges() {
	...
	while(UsedGPoints.size() < GPoints.size() && MinDistance > 0) {
		...
		for(unsigned int j=0; j<UsedGPoints.size() && !MinDistance; j++) {
			for(unsigned int i=0; i<UsedGPoints[j]->NearPoints.size(); i++) {
		...
		//main points cycle
		for(unsigned int j=0; j<UsedGPoints.size(); j++) {
			//near points cycle
			for(unsigned int i=0; i<UsedGPoints[j]->NearPoints.size(); i++) {
