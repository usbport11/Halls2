...
template <typename T>
int HopsNumber(TNode<T>* NodeA, TNode<T>* NodeB, TNode<T>* parent = NULL, int Steps = 0) {
	if(!NodeA || !NodeB) return 0;
	if(NodeA == NodeB) return 0;
	if(NodeA->NeighborExist(NodeB)) return Steps + 1;
	typename std::list<TNode<T>* >::iterator itn;
	int res = 0;
	for(itn = NodeA->Neighbors.begin(); itn != NodeA->Neighbors.end(); itn++) {
		if(parent) {
			if(*itn == parent) continue;
		}
		res = HopsNumber(*itn, NodeB, NodeA, Steps + 1);
		if(res > 0) return res;
	}
	return 0;
}
