...
NVector2 NVector2AbsDelta(NVector2 A, NVector2 B) {
...
float NVector2Distance(NVector2 v1, NVector2 v2) {
	return (float)sqrt((v2.x - v1.x) * (v2.x - v1.x) + (v2.y - v1.y) * (v2.y - v1.y));
}
...
bool NLine2CrossPoint(NLine2 LineA, NLine2 LineB, float* Point) {
	...
	Point[0] = (float)(Delta.x/Determinant);
	Point[1] = (float)(Delta.y/Determinant);
	return true;
}
...
bool PointInRectangle(NRectangle2 rec, NVector2 point) {
	return (point.x >= rec.Position.x && point.x <= rec.Position.x + rec.Size.x && point.y >= rec.Position.y && point.y <= rec.Position.y + rec.Size.y);
}
